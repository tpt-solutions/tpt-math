...
`tpt-math-optimize-convex` (a convex QP interface over the
[`clarabel`](https://crates.io/crates/clarabel) interior-point solver) behind
Cargo features. Depend on this crate when you want both smooth unconstrained
minimisation and constrained convex programming from a single dependency line.
...
| `tpt-math-optimize-convex` | `convex` | [`tpt-math-optimize-convex`](https://docs.rs/tpt-math-optimize-convex) — convex QPs via `clarabel` |

Both leaf crates take their parameter vectors as `nalgebra` `DVector<f64>` /
`DMatrix<f64>` from the same `nalgebra` that `tpt-math-linalg` wraps, so values
move between the two solver families (and the rest of the workspace) without
conversion.
...
- `tpt-math-optimize-general` *(default)* — pulls in `tpt-math-optimize-general`
  ...
  taking `Options` and returning a `Solution`, plus the unchanged `argmin`,
  `argmin_math`, and `nalgebra` re-exports.
- `tpt-math-optimize-convex` *(default)* — pulls in `tpt-math-optimize-convex`
  ...
  builder, `QpSolution`, `ConvexError`, plus the unchanged `clarabel` and
  `nalgebra` re-exports.
...
This crate requires `std`; both `argmin` and `clarabel` are `std`-only, so there
is no `no_std` configuration of this umbrella.
...
```rust
use tpt_math_optimize::general::{minimize_gradient_descent, nalgebra::DVector};
...
```rust
use tpt_math_optimize::convex::{nalgebra::{DMatrix, DVector}, solve_qp};
...
- Nothing is hidden: `general` re-exports `argmin` (and `argmin::core`,
  `argmin::solver`, `argmin_math`) and `convex` re-exports `clarabel`, so you
  can drop to the upstream APIs without adding a second, possibly
  version-skewed, dependency on them.
...
- Upstream licences differ from this crate's: `argmin` is `MIT OR Apache-2.0`
  and `clarabel` is Apache-2.0. Review both when redistributing.
