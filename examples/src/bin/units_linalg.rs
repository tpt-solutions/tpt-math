...
fn main() {
    ...
    let offset = Vec::<Length>::from_raw(nalgebra::dvector![1.0_f64, 0.0]);
    let total = pos + offset;
    assert_eq!(total.raw(), &nalgebra::dvector![4.0, 4.0]);
    ...
    let v_vec = Vec::<Velocity>::from_raw(nalgebra::dvector![velocity.get::<meter_per_second>()]);

    // Matrix multiplication propagates the row/column unit tags:
    // `Mat<Length, Time> * Mat<Time, Velocity> = Mat<Length, Velocity>`.
    let m = Mat::<Length, Time>::from_raw(nalgebra::dmatrix![1.0_f64, 2.0; 3.0, 4.0]);
    let n = Mat::<Time, Velocity>::from_raw(nalgebra::dmatrix![1.0_f64, 0.0; 0.0, 1.0]);
